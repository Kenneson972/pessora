-- PESSORA — fn_save_post_registration_survey : bilan_offert n'est plus obligatoire.
--
-- Contexte : le retrait du parcours Bilan de la surface publique (bloc 1,
-- feat/retrait-bilan-surface-publique) a rendu bilan_offert optionnel côté
-- client (postRegistrationSurveySchema.ts) et neutralisé l'étape dans
-- PostRegistrationWizard.tsx — le champ n'est plus jamais envoyé. Mais cette
-- RPC exigeait encore une valeur non vide pour TOUS les types d'événements
-- (`missing_bilan_offert`), donc toute soumission du questionnaire
-- post-inscription échouait depuis le retrait, quel que soit le type
-- d'événement. 0 inscription en base au moment du fix (aucune donnée
-- perdue) — corrigé avant impact réel.
--
-- Le champ redeviendra pertinent au lot A (Challenge/Bilan) quand le
-- parcours sera rétabli avec la garantie serveur ; la RPC sera alors
-- ajustée pour l'exiger uniquement sur les événements type='challenge'.
...
BEGIN
  ...
  -- bilan_offert retiré de la validation obligatoire (voir commentaire d'en-tête).
