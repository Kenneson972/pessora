...
- **Challenge 21 jours** = rubrique **DANS la page Événements** (pas de page ni landing séparée) — **MISE À JOUR 12/09 : une route stable `/evenements/challenge-21-jours` a été ajoutée et rend LE MÊME composant (`ChallengeLanding`) que `/evenements/:slug` quand `type='challenge'`.** Ce n'est **pas** une page isolée : c'est une **seconde porte d'entrée** vers la même page, pour qu'un lien partagé (bio Instagram, QR au bar) survive **entre deux challenges**. La règle « pas de page marketing séparée » **tient toujours** — voir `docs/CONSIGNES-CLAUDE.md`, section « PAGE CHALLENGE » ; créneaux de bilan ouverts **J-14 → jour J**, calculés **à la lecture** (décision : pas de `pg_cron`) ; **inscription au bilan obligatoire** pour participer.
