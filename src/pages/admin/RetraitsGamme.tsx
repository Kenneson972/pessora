...
export default function RetraitsGamme() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    setOrders(grouped); setLoading(false);
  }, []);
  useEffect(() => { fetchAll(); document.title = 'Retraits Gamme — Admin PessÓra'; }, [fetchAll]);
              ...
              <div key={col.key} className={`rounded-[2px] border ${col.color} p-4`}>
                ...
                <div className="space-y-3">
                  {orders[col.key]?.map(order => {
                    ...
                    if (col.key === 'paid') return <PaidCard key={order.id} order={order} items={items} onUpdate={fetchAll} />;
                    return (
                      <div key={order.id} className="rounded-[2px] border border-noir/[0.06] bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.03)]">
                        <p className="text-[14px] font-bold text-black mb-1">{order.client_name || 'Client'}</p>
                        {order.client_phone && <p className="text-[11px] text-black/45 mb-2 flex items-center gap-1"><Phone size={11} />{order.client_phone}</p>}
                        {order.scheduled_pickup_date && (
                          <p className="text-[11px] font-medium text-sapin mb-2">
