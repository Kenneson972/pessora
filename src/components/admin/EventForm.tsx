              ...
              <div className="mt-4 flex flex-col gap-3 rounded-[2px] border border-noir/[0.06] bg-surface-muted/60 p-4">
                ...
                <div>
                  ...
                  {form.popup_image_url ? (
                    ...
                    <p className="text-[10px] text-black/40">Aucune image dédiée — la couverture de l'événement sera utilisée.</p>
                  )}
                  {popupImageError && <p className="mt-1.5 text-[10px] text-red-500">{popupImageError}</p>}
                </div>
                <div className="rounded-[2px] border border-noir/[0.05] bg-white px-3 py-2.5 text-[10px] leading-relaxed text-black/55"><p><span className="text-black/35">Lien : </span><span className="text-black/70">/evenements/{pathSlug}</span></p>{form.date && (<p className="mt-1"><span className="text-black/35">Expire après : </span><span className="text-black/70">{formatLongDate(form.date)}</span></p>)}</div>
