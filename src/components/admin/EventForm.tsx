      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_minmax(320px,420px)] lg:gap-10">
        <div className="flex flex-col gap-8">
          ...
          <section className="rounded-[2px] border border-noir/[0.06] bg-white p-5 sm:p-6">
            ...
            <p className="mt-2 text-[10px] text-black/60">{form.description.length} caractères · affiché sur la page publique.</p>
          </section>
          ...
          <section className="rounded-[2px] border border-noir/[0.06] bg-white p-5 sm:p-6">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div><label className={labelBase}>Capacité maximale</label><input type="number" className={inputBase} value={form.places_max} placeholder="Illimitée" onChange={(e) => set('places_max', e.target.value)} /><p className="mt-1.5 text-[10px] text-black/60">Laisser vide pour illimité.</p></div>
            </div>
          </section>

          {error && (<p className="rounded-[2px] border border-red-200 bg-red-50/60 px-4 py-3 text-[12px] text-red-700">{error}</p>)}
        ...
        <aside className="flex flex-col gap-6 lg:sticky lg:top-[4.5rem] lg:self-start">
          ...
          <section className="rounded-[2px] border border-noir/[0.06] bg-white p-5 sm:p-6">
            ...
            <div className="flex flex-col gap-3">
              <label className="flex items-start justify-between gap-4 rounded-[2px] border border-noir/[0.06] bg-surface-muted px-3.5 py-3 cursor-pointer"><span><span className="block text-[12px] font-normal text-noir">Visible en ligne</span><span className="mt-0.5 block text-[10px] text-black/60">L'événement apparaît sur la page publique /evenements.</span></span><input type="checkbox" checked={form.active} onChange={(e) => set('active', e.target.checked)} className="mt-0.5 h-4 w-4 accent-black rounded-[2px]" /></label>
              <label className="flex items-start justify-between gap-4 rounded-[2px] border border-noir/[0.06] bg-surface-muted px-3.5 py-3 cursor-pointer"><span><span className="block text-[12px] font-normal text-noir">Inscriptions ouvertes</span><span className="mt-0.5 block text-[10px] text-black/60">Les visiteurs peuvent réserver une place.</span></span><input type="checkbox" checked={form.registration_open} onChange={(e) => set('registration_open', e.target.checked)} className="mt-0.5 h-4 w-4 accent-black rounded-[2px]" /></label>
            </div>
          </section>
          <section className="rounded-[2px] border border-noir/[0.06] bg-white p-5 sm:p-6">
            <DashEyebrow className="mb-1">Pop-up d'accueil</DashEyebrow>
            <p className="mb-4 text-[10px] leading-relaxed text-black/60">Affiche un pop-up sur la page d'accueil et /evenements pour mettre en avant cet événement. Optionnel — les champs sont pré-remplis depuis l'événement.</p>
            <label className="flex items-start justify-between gap-4 rounded-[2px] border border-noir/[0.06] bg-surface-muted px-3.5 py-3 cursor-pointer"><span><span className="block text-[12px] font-normal text-noir">Afficher un pop-up</span><span className="mt-0.5 block text-[10px] text-black/60">{form.popup_enabled ? form.popup_id ? 'Pop-up lié à cet événement.' : 'Un pop-up sera créé au moment de l\'enregistrement.' : form.popup_id ? 'Le pop-up sera supprimé à l\'enregistrement.' : 'Aucun pop-up n\'est rattaché pour l\'instant.'}</span></span><input type="checkbox" checked={form.popup_enabled} onChange={(e) => { if (e.target.checked) enablePopup(); else set('popup_enabled', false); }} disabled={!popupLoaded} className="mt-0.5 h-4 w-4 accent-black rounded-[2px]" /></label>
            {form.popup_enabled && (
              <div className="mt-4 flex flex-col gap-3 rounded-[2px] border border-noir/[0.06] bg-surface-muted/60 p-4">
                <label className="flex items-start justify-between gap-3"><span><span className="block text-[11px] font-normal text-noir">Actif en ligne</span><span className="mt-0.5 block text-[10px] text-black/60">Décocher pour préparer le pop-up sans le publier.</span></span><input type="checkbox" checked={form.popup_active} onChange={(e) => set('popup_active', e.target.checked)} className="mt-0.5 h-4 w-4 accent-black rounded-[2px]" /></label>
          ...
          {existing && (<section className="rounded-[2px] border border-noir/[0.06] bg-white p-5 sm:p-6"><DashEyebrow className="mb-3">Statistiques</DashEyebrow><div className="flex items-baseline justify-between"><div><div className="font-display leading-none" style={{ fontFamily: 'var(--font-display)', fontSize: 32 }}>{count}</div><p className="mt-1 text-[10px] uppercase tracking-[0.22em] text-black/60">Inscrit{count > 1 ? 's' : ''}</p></div>{form.places_max && (<p className="text-[10px] text-black/60">/ {form.places_max} places</p>)}</div></section>)}
        </aside>
      </div>
    </motion.div>
  );
