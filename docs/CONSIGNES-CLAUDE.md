...
- **Recette de cette garde** : POST anon avec `challenge_event_id` = un événement **non-challenge** → **refus `P0004`** · avec un challenge → **201** · puis `SELECT count(*) FROM bilan_bookings b JOIN events e ON e.id = b.challenge_event_id WHERE e.type <> 'challenge'` → **0**.
