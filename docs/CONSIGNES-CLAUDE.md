...
- 🔒 **Critères de recette de cette passe (contre-signature @vela — écrits AVANT la passe, pas après)** :
  1. **zéro occurrence de `pessora.fr@gmail.com` dans TOUT le repo**, `docs/` comprise (`grep -rn` sur la racine, hors `node_modules`) ;
  2. **`template/` ne contient plus AUCUNE coordonnée réelle** de Catherine — ni adresse, ni lien Maps, ni e-mail (`contact@exemple.fr`, `00000`, Maps vide) ;
