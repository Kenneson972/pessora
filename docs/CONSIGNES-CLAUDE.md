...
- **Garde à poser DANS CE LOT** (même doctrine que le rattachement du créneau) : un trigger `BEFORE INSERT OR UPDATE` sur `bilan_bookings` qui **refuse** (ou annule) tout `challenge_event_id` dont l'événement n'est pas `type='challenge'` **et** `active=true`. La RPC, elle, fournit un id **valide** — la validation par type lui suffit, aucun GUC supplémentaire n'est nécessaire.
- ⚠️ **Rien en base ne garantit aujourd'hui que `challenge_event_id` pointe vers un `challenge`** : la clé étrangère ne référence que `events(id)`, sans contrôle de `type`. La règle repose donc **sur la RPC seule**. Contrôle de recette à jouer (à côté du ⑨) : `SELECT count(*) FROM bilan_bookings b JOIN events e ON e.id = b.challenge_event_id WHERE e.type <> 'challenge'` → **0**. Et **dette écrite** : poser une garde **en base** (trigger `BEFORE INSERT OR UPDATE` vérifiant le `type`) le jour où un **second écrivain** apparaît — c'est exactement le motif « une colonne, un seul écrivain » qui a produit le créneau orphelin du lot A.
