...
3. **Les vérifications post-merge — ✅ FAITES, mesurées EN SERVICE (12/09)** : **`/admin` seul** → `noindex, nofollow` ✅ (le trou du `/admin` nu est fermé **en prod**) · `/admin/dashboard` → `noindex` ✅ (non-régression) · **sitemap servi** : 200, **0 apex**, **62 `<loc>` de pages en `www`** ✅ — ⚠️ le fichier porte **109 `<loc>`** : **62 pages + 47 `image:loc`** (namespace images, non touché) : **ne pas lire cet écart comme un bug** ✅ · les **3 assertions `X-Robots-Tag`** ✅ · l'**OG servie** en 1200×630 ✅ · **4 URLs de pages prises au hasard** → 200 ✅.
4. **⏳ LE BALAYAGE (« marche 6 » du test e2e) — lot BACKEND, décidé mais NON CODÉ, et il attend un GO.** Ce n'est pas une correction : c'est **un lot à part entière** (l'e-mail est le canal le plus sensible). **①** une **migration** — `notified_at timestamptz` sur `bilan_bookings` : **la colonne n'existe pas** aujourd'hui (vérifié en base — donc ce n'est pas « juste du code »), et les **3 chemins** (questionnaire, créneau, hors-créneau) y convergent : c'est **la « demande » à notifier** ; **②** l'**edge function de balayage** — scan `notified_at IS NULL`, envoi Resend (code de `send-contact-email`), **marquage APRÈS envoi réel**, **`service_role`** + **`verify_jwt = true`** ; **③** le **cron 15 min** + la **trace** lue par le scan de 8 h (critère : « le job **voit** la ligne », jamais `exit 0`). Le design est **écrit** (chemin, marquage après envoi, `service_role`, 1 e-mail par demande avec le nom en objet) → **rien à re-décider, juste à coder** — **après un go explicite**.
   - ⚠️ **ET DEUX CONTRAINTES QUE LE DESIGN N'AVAIT PAS ÉCRITES** (@vela, vérifiées dans le code) :
     - **(a) PURGER LES FIXTURES AVANT DE DÉMARRER LE CRON.** Le premier balayage scanne `notified_at IS NULL` → il prend **tout ce qui est en table**, **y compris les lignes de recette** ✅. Donc **le premier tick enverrait de vrais e-mails à propos de fixtures** ✅. À ajouter à l'**inventaire de purge**, et à faire **avant** le premier passage — pas après.
     - **(b) LE MARQUAGE EST SERVEUR.** Les **deux inserts** du widget (`BilanBookingWidget`, l.102 créneau et l.136 hors-créneau) **ne font délibérément pas** de `.select()` → **le front ne voit jamais la ligne** ✅. Donc **c'est le job qui marque `notified_at`**, jamais le front ✅.
...
- 🔴 **ET L'ICÔNE MANQUE DANS LES DEUX ÉTATS — à câbler maintenant** (@vela, vérifié dans le code poussé `4048075`) : `ChallengeProgramCard.tsx` rend aujourd'hui `{item.image ? <img/> : <div style={{background: BANNER_FALLBACK}}/>}` suivi du libellé — **aucune icône**, et la liste `INCLUS` ne porte qu'un `label` et une `image?` : **le lien libellé → icône n'existe pas dans le code**. Conséquence : la porte « 6 icônes dans le DOM » échouerait **avant même la première bannière** (0 au lieu de 6), et la livraison produirait une **grille à deux vitesses** — exactement ce qu'@lyra voulait éviter.
  - ✅ **LES 6 ICÔNES : Lucide, DÉJÀ dans le projet — on n'importe, on ne copie pas** (@user a demandé « de beaux SVG plutôt que d'en créer » ; il y a mieux que copier : **`lucide-react@0.507.0` est déjà la bibliothèque du site**, comme `Check`, `ArrowRight` ou `CalendarDays`). Même grammaire, même épaisseur, **aucun dessin à maintenir**, licence **ISC** (libre, usage commercial compris).
    ...
    - ⚠️ **`stroke-width` : garder `1.4`** (celui de la maquette) — le défaut de Lucide est `2`, plus lourd ; **@vela a vérifié que `1.4` rend correctement** à cette taille.
    - ✅ **Les 6 passent à 20 px — mesurés sur les SVG RÉELS de `lucide-react@0.507.0` installé** (@vela), sur le **dégradé de repli** et sur **zone claire sous voile**, à la taille exacte. **Conformité : 5 comptages à 0** — aucune balance, aucun mètre ruban, aucune courbe de poids, aucun écran d'interface, aucun corps humain ✅. `Smartphone` = un **contour de téléphone avec un point** : un objet, exactement ce qu'on voulait ✅. `Salad` est **le moins net des six**, mais **identifiable** ✅. Et `Users` + `Dumbbell` sont **plus lisibles que les tracés maison** qui blobaient : le passage à Lucide est un **gain**, pas un compromis ✅.
    - ⚠️ **LE SEUL CAS OÙ ON CHANGE — et c'est UNE LIGNE, pas une redécision.** Le verdict ci-dessus est rendu sur le **repli** et un fond clair **plat** ; sur une **vraie photo texturée**, `Salad` **pourrait** devenir confus (seul candidat restant). Si ça arrive : **`Carrot`, `Utensils` ou `CookingPot`** — **vérifiés présents dans la 0.507.0 installée** ✅. Idem pour `MessagesSquare` → `MessageCircle` en secours. **On remplace le nom dans l'import, rien d'autre.** *(À rejouer sur la première bannière livrée, avec la porte au pixel.)*
  ...
  - 🔴 **ET LA COULEUR DU TRAIT — l'icône est SUPERPOSÉE au voile sombre, comme le libellé.**
    - ⚠️ **Formulation corrigée le 12/09** : une première version disait « *l'icône est toujours sur fond sombre → jamais `sapin`* ». C'était **ambigu** — @lyra a failli en conclure « alors sapin » (le fond clair de la carte), et **six icônes invisibles** seraient parties au premier commit. Ce qui suit lève l'ambiguïté.
    - **La règle de fond : le SVG ne porte JAMAIS de couleur en dur** → `stroke="currentColor"` ✅. **La couleur vient du CSS.**
    - **Et la couleur du CSS, c'est du CLAIR** (ivoire/écru, `#F7F4ED`) — **parce que l'icône est posée PAR-DESSUS le voile sombre**, exactement comme le libellé. La mise en page est en `absolute inset-0` : l'image **et** le dégradé de repli couvrent **toute** la carte, et le contenu passe **au-dessus** ✅.
    - **La preuve que ce n'est pas « le fond clair de la carte »** : le libellé est en **`text-white`** **aujourd'hui** — s'il était posé sur le fond clair de la carte, il serait **déjà** invisible ✅.
    - **Mesuré** : trait **`sapin` sur le voile = 1,24 → 1,45:1** (invisible) · trait **clair = 14,85:1** ✅. Et **aujourd'hui les six cartes affichent le repli** → livrer du `sapin`, c'est livrer **six icônes invisibles dès le premier commit** ✅.
    - **Porte (@vela)** : contraste du trait sur le **pixel réel** derrière l'icône — **≥ 3:1**, dans les **deux** états. Mesure du pixel, pas de la théorie : les deux erreurs symétriques (**blanc sur blanc**, **sombre sur sombre**) tombent, quelle que soit l'hypothèse de départ ✅.
    - ⚠️ **Et le voile : +24 px recommandés.** L'icône se pose à **~50 px du bas**, là où le voile faiblit (α ≈ 0,43 en haut de l'icône) → au **pire cas** (photo blanche dessous), son sommet tombe à **3,07:1** sur desktop ✅ — au-dessus du seuil, mais **la marge est mince** : une carte **~5 % plus courte** (≈ 232 px) le ferait passer **sous 3:1** ✅, et ça arriverait le jour où quelqu'un touche au `max-w` ou au padding **sans penser aux icônes**. Étendre le voile de 24 px ne coûte rien, n'a **aucun** effet sur les six bannières, et remonte à **~4,4:1** ✅.
    - ⚠️ **Et le fichier `icones-inclus-21j.md` porte encore `stroke: var(--color-sapin)`** — il partait du principe que « les mêmes fichiers serviront en clair **et** en sombre ». **@lyra le corrige** dans son fichier ; ici, la règle fait foi.
...
>
> ⚠️ **Il y a un QUATRIÈME point OG, et il n'est PAS une réparation** (@alcyone) : le **câblage de l'OG du challenge** — `og-challenge-1200x630.jpg` (le **JPEG de 61 Ko**, jamais le PNG de 380 Ko) posé comme `og:image` de la route challenge via une **entrée dédiée dans `seoConfig.ts`** (jamais à la place du défaut, `seoConfig` est **par page**), **dimensions déclarées = dimensions du fichier** (1200×630 ✅). ⚠️ **Décision RÉVISÉE le 12/09 : cette entrée se pose dans `fix/seo-og-share`**, pas sur la branche du challenge — elle touche `seoConfig.ts`, **déjà modifié par cette branche** : **un seul écrivain par fichier**, et l'aperçu de lien se vérifie dans **la même passe** que les 4 `apex → www`. Sans cette ligne, l'image de @user reste **un fichier sur le disque que personne ne sert**. À vérifier dans la spec de la landing. *(Bilan : **trois** réparations de prod sur `fix/seo-og-share`, **une** entrée neuve sur la landing.)*
...
⚠️ **Deux points restent ouverts, hors de cette porte :** **(a)** @lyra a relevé que **« ÓRA+ » est toujours dans la navigation de l'espace membre** (sidebar + barre mobile) → **passe Óra+** ; **(b)** les **7 créneaux legacy** de `bilan_slots` (avril/mai, passés, 0 réservation) s'affichent **en tête de l'onglet Créneaux** de Catherine — ⚠️ **ce n'est PAS du test : c'est du legacy, et ça demande un SECOND go** (@alcyone, 12/09). Ils n'ont **pas** le préfixe `TEST-`, datent d'**avril/mai** et n'ont **jamais été réservés** (`disponible=true` partout) → **c'est peut-être la vraie planification initiale de Catherine**, posée avant que le système challenge soit finalisé puis laissée en plan. **Les supprimer d'un bloc avec `TEST-KEN` serait propre sur le papier et faux sur la donnée** : on effacerait un historique qu'on ne sait pas nommer. Donc **deux décisions séparées** : `TEST-KEN` (test `<TEST->`, préfixe sans ambiguïté) sur le go de Ken ✅ ; **les 7 legacy seulement après confirmation de Catherine** ✅. *(@nova : tant qu'ils sont là, toute porte « 0 orphelin » est rouge par construction — c'est une raison de plus de trancher, pas de trancher vite.)*
...
- 🔴 **SEO / OG — trois corrections PRÉEXISTANTES à faire dans la même passe** (@alcyone, chemins et comptage revérifiés par @elise) :
  ...
  - **③ `seoConfig.ts` est PAR PAGE** (`l.22` défaut, `l.37` « La Carte ») → `og-challenge-1200x630.jpg` doit être **une entrée propre pour la route challenge**, **pas** un remplacement du défaut : sinon **tout le site** partagerait l'image du challenge.
...
- 🔴 **DEUX IMPLÉMENTATIONS DE LA MÊME FONCTIONNALITÉ — une seule peut gagner (@nova, 12/09).** Vérifié dans les deux branches :
  ...
  **Elles ne cohabitent pas** : mergées toutes les deux → **deux grilles** (ou un conflit sur `ChallengeLanding.tsx`). ⚠️ **Et c'est le layout B que Ken a validé avec Catherine** (« c'est comme ça que je veux », images **générées par lui** avec Higgsfield) → **donc B gagne** : son layout, ses 6 emplacements, ses fichiers. **`feat/bannieres-inclus` ne se merge pas** (on peut garder la découpe d'@lyra comme **source** d'une variante 4:3, pas comme câblage).
  ⚠️ **Conséquences à appliquer** : **①** la « spec des bannières » plus bas décrit le **layout A** (carte 4:3, 325 px, `INCLUS`, mesures de voile) → **obsolète pour B** (bande pleine largeur, voile horizontal) ; **②** l'`onError` discuté ce matin se pose dans **`ChallengeInclusBanners.tsx` (l.82, le `<img>`)**, **pas** dans `ChallengeProgramCard` ; **③** état actuel (corrigé le 12/09) : **les 6 bannières existent** (la 6ᵉ — « suivi des objectifs » — est dans la branche, `3f09c5a`) ✅, et le conflit est **TEXTUEL**, pas seulement visuel : les deux branches touchent **`ChallengeProgramCard.tsx`**, **le seul fichier commun** → les merger ensemble fait **échouer git** (@alcyone, vérifié).
  ⚠️ **Fragilité mesurée (@vela) — à trancher par @lyra, avec une garantie structurelle :** le **bloc de texte va jusqu'à 80 % de la largeur** alors que le **scrim s'efface dès 65 %** (≈ 0,07 à cet endroit) → **les derniers mots reposent sur une image quasi non voilée**. Ça se lit sur les 6 images actuelles (leurs zones droites sont sombres) **mais la marge est nulle** : une image un peu plus claire demain et la fin de la description devient illisible. Le correctif **robuste** n'est pas une retouche d'image : le **scrim doit couvrir la largeur réelle du bloc texte**, et la **porte** doit mesurer le contraste **au bord DROIT du texte**, sur **chaque breakpoint** — pas seulement à son début.
  ⚠️ **Et un seul point sur les images (décision @user, branche non mergée) :** `communaute` porte **un logo vert circulaire sur le gobelet café** qui **ressemble à une marque connue** → se corrige **en re-générant**, pas en recadrant.
  ⚠️ **Correction du 12/09 (@elise, vérifié à l'image) : le point « le gobelet rouge sort du cadre » NE concerne PAS la branche B.** L'image que `ChallengeInclusBanners.tsx` pointe pour `conseils` est `public/challenge-21j/inclus-conseils-accompagnement.webp` (**31 334 o**) : c'est **le salon vide** — le carnet, le stylo, **le marbre clair**. **Aucun gobelet.** Le gobelet rouge appartient à `banniere-conseils-4x3.jpg`, la **découpe d'@lyra dans la branche A** (`feat/bannieres-inclus`), **qui ne sera pas mergée** — les deux ont été regardées ensemble parce qu'elles traitent le même libellé. **Ne pas « corriger » un gobelet absent** : le **seul** correctif du bandeau `conseils` est **le scrim** (c'est le marbre clair qui produit le 2,85:1 mesuré par @vela).

- 🧹 **`src/pages/BilanBienEtre.tsx` — page PUBLIQUE non routée, à supprimer (pas à rebrancher).** Vérifié le 12/09 : aucun `Route` ne la monte, et c'est **la seule page publique** dans ce cas (les 18 autres fichiers non cités dans `App.tsx` sont des **pages admin**, routées ailleurs). Elle porte encore **un texte qui promet** : *« L'équipe PessÓra te confirme ton rendez-vous par **WhatsApp sous 24 h** »* — un parcours qui **n'existe plus** (le choix du créneau a migré dans la page Événement, décision du RDV). → **à supprimer dans la passe de nettoyage** : du code mort **avec une promesse fausse dedans**, c'est exactement ce qu'un futur dev « rebranche » un jour en croyant bien faire — *le fichier que personne ne sert, mais qui dit quelque chose de faux.*
