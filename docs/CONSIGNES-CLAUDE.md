...
- 🚫 **Claude écrit le CODE — il ne touche JAMAIS à la base de production.** Aucune migration ne s'applique depuis son poste ni par un outil connecté (serveur MCP Supabase, `supabase db push`, PAT du projet). Toute application passe par **@alcyone, en une transaction**, avec **contrôle par objet** (`prosrc`, `pg_trigger`, `pg_policies`).
  - *Motif (11/09)* : une migration **`20260911125459`** — absente de **tout** fichier du repo — a été appliquée en production hors de ce circuit, avec une version **à mi-lot** du travail en cours (fonction créée, jamais appelée ; base intacte par chance). Et **`supabase_migrations.schema_migrations` ne dit pas l'état de la base** : elle **ignore** nos applications par API Management (les 3 du lot A n'y sont pas) et **enregistre** celle-là. Elle est fausse **dans les deux sens**.
  - Conséquence de méthode : après chaque application, **vérifier qu'aucune migration inconnue n'est apparue** dans la table de suivi depuis le dernier contrôle.
