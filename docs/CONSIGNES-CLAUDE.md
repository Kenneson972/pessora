...
3. **✅ `fix/seo-og-share` existe maintenant** (`6befb92`, 1 commit) : les **3 réparations de prod** sont dedans (`index.html`, `PageSEO.tsx`, `seoConfig.ts`). **À recetter puis merger** — les 3 assertions du contrôle sont dans la section SEO, avec leur mesure « avant ». ✅ **Vérifié : elle ne partage AUCUN fichier avec les deux autres branches** (`x-robots-tag` = `vercel.json` ; challenge = `src/components/events/*`, `App.tsx`, pages, libs) → **aucun conflit de merge, ordre libre**, et elle peut partir **sans attendre les portes de la landing**.
