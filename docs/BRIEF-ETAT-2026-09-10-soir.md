...
- 🧹 **À purger au go-live (s'ajoute à l'inventaire)** : **8 inscriptions de test** `TEST-VELA` / `TEST-VELA-NEG` (tél. `0696000000` → `0696000007`) + l'événement `RUN CLUB - MARIN` a été remis à `registration_open = false` (vérifié).
...
4. ✅ **Bloc Partenariat / page Contact** : **poussé** (`feat/bloc-partenariat-page-contact`, `Contact.tsx` seul, +31/−10). Review OK : le CTA mène à `/contact-partenariat` (page existante), l'ancien texte inline est remplacé, la pill retirée au lot F n'est pas réintroduite. → **en file pour la recette**, derrière le lot A.
5. ✅ **Fix RPC `bilan_offert`** (`fix/rpc-bilan-offert-non-obligatoire`, `3287726`) : le retrait du Bilan avait cassé les **inscriptions aux événements** — la RPC exigeait encore `bilan_offert` alors que le client ne l'envoie plus. **Reproduit en prod** (400 `missing_bilan_offert`), migration `20260910180000` **appliquée** (fonction vérifiée par son `prosrc`, `SECURITY DEFINER` + `search_path` préservés), après fix : **200** — et les **5 contrôles négatifs** toujours en **400** (téléphone, objectif, clés `run_club`, inscription inconnue). → **feu vert, à merger**.
