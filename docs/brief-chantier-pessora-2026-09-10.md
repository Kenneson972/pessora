...
- 🔴 **LES EDGE FUNCTIONS NE SONT PAS DÉPLOYÉES PAR VERCEL** (déploiement manuel via CLI — aucun workflow GitHub dans le repo). Au 10/09, `create-checkout-session` en prod = **v22 du 09/09 17:09** → **toutes les modifs serveur du bloc 1 restent hors ligne** tant qu'on ne déploie pas.
  ...
  - ⚠️ **Fenêtre dangereuse** : front avec boosters **2 €** + serveur à **1 €** → la vérification anti-fraude (±0,02 €) **rejette le panier** → ventes cassées (idem remise Óra+ encore appliquée côté serveur). ➡️ **Merge des branches prix + déploiement des fonctions = une seule opération**, suivie de la recette.
